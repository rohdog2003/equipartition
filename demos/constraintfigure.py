...
plt.xlabel(r"4-Velocity $u=\beta\Gamma$")
plt.xlim(0, 25)
...
for i in range(len(tctildedivt)):
    if i == 2:
        ...
        plt.annotate(r"$u_{\rm{on, lb}}$", (uonlb[i] + 0.20, -0.35), rotation = 90)
        ...
        plt.annotate(r"$u_{\rm{off, ub}}$", (uoffub[i] + 0.20, -0.35), rotation = 90)
        plt.vlines(ucmin, ymin = -0.4, ymax = Equipartition.constraint_gammaBeta_corr(ucmin, t, tctilde[i], pb), color = colors[i], linestyle = "--")
        plt.annotate(r"$u_{\rm{min}}$", (ucmin + 0.20, -0.35), rotation = 90)
        plt.plot(uon, 0, color = colors[i], marker = "o")
        plt.annotate(r"$u_{\rm{on}}$", (uon, 0), xytext = (1, 3), textcoords = "offset points")
        plt.plot(uoff, 0, color = colors[i], marker = "s")
        plt.annotate(r"$u_{\rm{off}}$", (uoff - 1.6, 0), xytext = (1, 3), textcoords = "offset points")
